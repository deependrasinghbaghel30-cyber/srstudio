...
export default async function handler(req) {
  ...
  const { mode, engine, prompt, imageDataUrl, imageDataUrls, images, aspect } = body || {};
  ...
  if (mode === 'image') {
    ...
    for (const entry of imgEntries) {
      const input = { prompt: entry.prompt, duration: '5', aspect_ratio: ratio, generate_audio: true };
      if (eng === 'kling') input.start_image_url = entry.dataUrl; else input.image_url = entry.dataUrl;
      jobs.push(input);
    }
  } else {
    jobs.push({ prompt: defaultPrompt, duration: '5', aspect_ratio: ratio, generate_audio: true });
  }
