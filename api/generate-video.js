...
export default async function handler(req) {
  ...
  if (mode === 'image') {
    ...
    for (const entry of imgEntries) {
      const input = { prompt: capPrompt(entry.prompt), duration: clipDuration, aspect_ratio: ratio };
    ...
    for (const sceneText of list) {
      const input = { prompt: capPrompt(sceneText), duration: clipDuration, aspect_ratio: ratio };
      if (wantAudio) input.generate_audio = true;
      jobs.push(input);
    }
  }
...
// Video models reject prompts over 2500 characters — trim rather than fail the job.
const MAX_PROMPT = 2400;
function capPrompt(text) {
  const t = String(text || '').trim();
  return t.length > MAX_PROMPT ? t.slice(0, MAX_PROMPT) : t;
}
